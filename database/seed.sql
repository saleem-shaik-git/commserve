...
-- Opening balances are represented in the ledger so the ledger remains the source of truth.
INSERT INTO transactions (reference,type,status,amount,currency,description,completed_at)
SELECT CONCAT('OPEN-',a.account_number),'deposit','completed',a.available_balance,'USD','Opening balance',CURRENT_TIMESTAMP
FROM accounts a
