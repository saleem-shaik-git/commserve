...
<a href="<?=url('accounts.php')?>" class="btn btn-outline-secondary">Back to accounts</a>
<?php require __DIR__ . '/partials/footer.php'; exit; endif; ?>

<div class="d-flex justify-content-between align-items-center mb-4">
  <div>
    <nav aria-label="breadcrumb"><ol class="breadcrumb mb-1"><li class="breadcrumb-item"><a href="<?=url('accounts.php')?>">Accounts</a></li><li class="breadcrumb-item active"><?= e($account['account_number']) ?></li></ol></nav>
  ...
  <div class="d-flex gap-2">
    <a href="<?=url('transfer.php')?>?from=<?= $account['id'] ?>" class="btn btn-primary"><i class="bi bi-send me-2"></i>Transfer</a>
    <a href="<?=url('statements.php')?>?account=<?= $account['id'] ?>" class="btn btn-outline-primary"><i class="bi bi-file-earmark-text me-2"></i>Statement</a>
  </div>
</div>
    ...
    <div class="card border-0 shadow-sm">
      <div class="card-body p-4">
        ...
        <div class="d-grid gap-2 mt-4">
          <a href="<?=url('transactions.php')?>?account=<?= $account['id'] ?>" class="btn btn-outline-primary"><i class="bi bi-clock-history me-2"></i>Full Transaction History</a>
          <a href="<?=url('statements.php')?>?account=<?= $account['id'] ?>" class="btn btn-outline-secondary"><i class="bi bi-file-earmark-arrow-down me-2"></i>Download Statement</a>
        </div>
      </div>
    </div>
  ...
  <div class="col-lg-8">
    <div class="card border-0 shadow-sm">
      <div class="card-header bg-white d-flex justify-content-between align-items-center">
        ...
        <a href="<?=url('transactions.php')?>?account=<?= $account['id'] ?>" class="btn btn-sm btn-outline-primary">View all</a>
      </div>
    ...
    <div class="card border-0 shadow-sm mt-4">
      ...
      <div class="card-body">
        ...
        <div class="mt-3 d-flex gap-2">
          <a href="<?=url('statements.php')?>?account=<?= $account['id'] ?>&from=<?= date('Y-m-01') ?>&to=<?= date('Y-m-t') ?>&format=pdf" class="btn btn-sm btn-outline-secondary">This Month PDF</a>
          <a href="<?=url('statements.php')?>?account=<?= $account['id'] ?>&from=<?= date('Y-m-01') ?>&to=<?= date('Y-m-t') ?>&format=csv" class="btn btn-sm btn-outline-secondary">This Month CSV</a>
        </div>
      </div>
    </div>
  </div>
