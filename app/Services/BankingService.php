...
final class BankingService
{
    ...
    public function withdraw(int $accountId, string $amount, string $description = 'Simulated withdrawal', ?int $actorUserId = null, ?string $idempotencyKey = null): string
    {
        ...
        try {
            if ($idempotencyKey !== null) { $existing=$this->security->assertIdempotency($idempotencyKey); if($existing)return $existing; }
    ...
    public function transfer(int $fromAccountId, int $toAccountId, string $amount, string $description = 'Internal transfer', ?int $userId = null, ?string $transactionPin = null, ?string $idempotencyKey = null): string
    {
        ...
        if ($transactionPin === null || $transactionPin === '') throw new RuntimeException('Transaction PIN is required.');
        $this->security->verifyTransactionPin($userId, $transactionPin);
        $this->security->assertTransferAllowed($userId, $amount);
        $this->db->beginTransaction();
        try {
            if ($idempotencyKey !== null) { $existing=$this->security->assertIdempotency($idempotencyKey); if($existing){$this->db->rollBack();return $existing;} }
            ...
            $this->completeTransaction($reference);
            $this->recalculateBalance($fromAccountId); $this->recalculateBalance($toAccountId);
            $this->recordEvent($reference, 'transfer_completed', 'processing', 'completed', $userId);
            $this->db->commit(); return $reference;
        } catch (Throwable $e) { if ($this->db->inTransaction()) $this->db->rollBack(); throw $e; }
    }
    ...
    private function singleAccountMovement(int $accountId,string $amount,string $type,string $entryType,string $description,?int $actorUserId,?string $idempotencyKey):string{
        $this->assertPositiveAmount($amount);$this->db->beginTransaction();try{if($idempotencyKey!==null){$existing=$this->security->assertIdempotency($idempotencyKey);if($existing){$this->db->rollBack();return $existing;}}$account=$this->lockAccount($accountId);if($account['status']!=='active')throw new RuntimeException('Account is not active.');$reference=$this->createTransaction($type,$amount,$account['currency'],$description,$actorUserId,$idempotencyKey);$this->addLedgerEntry($reference,$accountId,$entryType,$amount);$this->completeTransaction($reference);$this->recalculateBalance($accountId);$this->db->commit();return $reference;}catch(Throwable $e){if($this->db->inTransaction())$this->db->rollBack();throw $e;}}
    ...
    private function addLedgerEntry(string $reference,int $accountId,string $entryType,string $amount):void{$stmt=$this->db->prepare('SELECT id FROM transactions WHERE reference=?');$stmt->execute([$reference]);$transactionId=$stmt->fetchColumn();$stmt=$this->db->prepare('INSERT INTO ledger_entries(transaction_id,account_id,entry_type,amount) VALUES(?,?,?,?)');$stmt->execute([$transactionId,$accountId,$entryType,$amount]);}
    private function recalculateBalance(int $accountId):void{$balance=$this->balance($accountId);$this->db->prepare('UPDATE accounts SET available_balance=? WHERE id=?')->execute([$balance,$accountId]);}
    ...
    private function recordEvent(string $reference,string $type,string $old,string $new,int $actor):void{$stmt=$this->db->prepare('SELECT id FROM transactions WHERE reference=?');$stmt->execute([$reference]);$id=$stmt->fetchColumn();$stmt=$this->db->prepare('INSERT INTO transaction_events(transaction_id,event_type,old_status,new_status,actor_user_id) VALUES(?,?,?,?,?)');$stmt->execute([$id,$type,$old,$new,$actor]);}
